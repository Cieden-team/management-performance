...
const DashboardPage = () => {
  ...
  const goals = useQuery(api.goals.getGoalsByUser, 
  ...
  // Mock skills data (поки що немає API для skills)
  const skills = [
    { 
      id: 1,
      name: "React", 
  ...
  // Loading state
  if (!currentUser || !goals || !feedbacks) {
    return (
      <Layout>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition-all duration-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900">
                  {feedbacks.length > 0 
                    ? (feedbacks.reduce((acc: number, f: any) => acc + (f.rating || 0), 0) / feedbacks.length).toFixed(1)
                    : "0.0"
                  }/5
                </p>
              </div>
